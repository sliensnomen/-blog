...
def release_port(port):
    try:
        import signal
        result = subprocess.check_output(f'lsof -ti:{port}', shell=True).decode()
...
if __name__ == "__main__":
    ...
    storage_dir = os.path.join(EXE_DIR, '.pywebview_storage')
    os.makedirs(storage_dir, exist_ok=True)
    try:
        webview.start(debug=True, storage_path=storage_dir)
