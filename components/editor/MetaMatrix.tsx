...
interface Props {
  type: 'post' | 'chatter' | 'about' | 'moment';
...
export default function MetaMatrix({
  type, tags, setTags, cover, setCover, summary, setSummary, mood, setMood, location, setLocation,
    ...
    <div className="flex flex-col h-full overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8 flex flex-col gap-10">
        ...
        <div className="flex flex-col">
          <Label icon={ImageIcon} text={type === 'moment' ? "Attached Images" : "Cover Image"} color="border-indigo-500" />
        ...
        {/* 👇 说说专属：位置 */}
        {type === 'moment' && setLocation && (
          <div className="flex flex-col animate-in fade-in slide-in-from-bottom duration-500">
            <Label icon={Clock} text="Location" color="border-emerald-500" />
            <input
              type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="你在哪里？"
              className="w-full bg-white/10 dark:bg-black/20 rounded-2xl px-5 py-4 text-xs text-slate-800 dark:text-slate-200 border border-white/10 outline-none focus:ring-2 focus:ring-emerald-500 shadow-inner"
            />
          </div>
        )}

        {/* 3. 心情区 (说说也支持心情) */}
        {(type === 'chatter' || type === 'moment') && setMood && (
