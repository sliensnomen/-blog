...
function EditorContent() {
  ...
  const handleSave = async (isPublish: boolean, shouldExitAfterSave: boolean = false) => {
    // 🌟 说说模式：不走 drafts API，直接调用 moments API
    if (isMoment) {
      const momentId = currentDocId === 'new' ? `moment-${Date.now()}` : currentDocId;
      const payload = {
        id: momentId,
      ...
      try {
        const configRes = await fetch(`/backend_config.json`);
        const config = await configRes.json();
        const res = await fetch(`http://127.0.0.1:${config.api_port}/api/moments/save`, {
        ...
        if (data.success) {
          ...
          showToast(isPublish ? "🚀 说说已发布！" : "💾 说说已保存", "success");
    ...
    if (docType === 'about') {
      ...
      try {
        const configRes = await fetch(`/backend_config.json`);
        const config = await configRes.json();
        // 直接调用 sync_local API 写入 about.md，格式要符合后端期望
    ...
    const payload = {
      id: currentDocId === 'new' ? null : currentDocId,
      ...
      <PageTransition>
        <main className="mx-auto w-[96%] max-w-[1750px] flex flex-row gap-6 relative" style={{ marginTop: '144px', height: 'calc(100vh - 144px - 32px)', marginBottom: '32px' }}>
          ...
          <aside className="w-[360px] shrink-0 bg-white/30 dark:bg-slate-800/40 backdrop-blur-[60px] rounded-[50px] shadow-2xl border border-white/30 dark:border-white/10 flex flex-col overflow-hidden">
            <MetaMatrix
              type={docType as any} tags={tags} setTags={setTags} cover={cover} setCover={setCover} summary={summary} setSummary={setSummary} mood={mood} setMood={setMood} location={location} setLocation={setLocation}
